...
npx wrangler d1 execute haber --local --command \
  "DELETE FROM article_tags; DELETE FROM article_sources; DELETE FROM articles;
   UPDATE site_state SET content_version = content_version + 1 WHERE id = 1;" >/dev/null 2>&1
